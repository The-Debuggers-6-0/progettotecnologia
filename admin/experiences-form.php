...
$data  = [
    ...
    'category_id' => '', 'location' => '', 'is_active' => 1,
];
$current_cover = null;
        ...
        try {
            if ($id === 0) {
                $stmt = db()->prepare(
                    'INSERT INTO experiences
                     (title, slug, description, short_description, price,
                      duration_minutes, max_participants, category_id, location, is_active)
                     VALUES (?,?,?,?,?,?,?,?,?,?)'
                );
                $stmt->execute([
                    ...
                    $data['category_id'], $data['location'], $data['is_active'],
                ]);
                ...
                $stmt->execute([
                    ...
                    $data['short_description'], $data['price'],
                    $data['duration_minutes'], $data['max_participants'],
                    $data['category_id'], $data['location'], $data['is_active'], $id,
                ]);
            }
